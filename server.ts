...
let splitwiseClient: any = null;

function getSplitwiseExport(): any {
  const mod: any = SplitwisePkg;
  return mod?.default ?? mod;
}

function buildSplitwiseClient(config: any) {
  ...
  const candidates = [
    ...
    SplitwisePkg as any,
    (SplitwisePkg as any)?.Splitwise,
    (SplitwisePkg as any)?.default,
  ].filter(Boolean);
  ...
  throw new Error(
    `Splitwise SDK init failed. Export keys: ${exportedKeys.join(", ")}. Last error: ${lastErr?.message || lastErr}`
  );
}

function getSplitwiseClient() {
  if (splitwiseClient) return splitwiseClient;
  ...
  if (!consumerKey || !consumerSecret) {
    throw new Error(
      "Splitwise is not configured. Set SPLITWISE_CONSUMER_KEY and SPLITWISE_CONSUMER_SECRET in .env"
    );
  }
...
async function findAccount(accountName: string): Promise<Account | null> {
...
function normalizeName(obj: any): string {
...
function extractBalances(friend: any): Array<{ currency_code: string; amount: number }> {
 ...
 */
function splitEquallyUsers(cost: number, payer_user_id: number, participant_user_ids: number[]) {
  const n = participant_user_ids.length;
  if (n <= 0) throw new Error("participant_user_ids must have at least 1 user_id");
  ...
  return {
    tools: [
      // -------------------- Existing Supabase Tools --------------------
      {
        ...
        description:
          "Record a new income or expense transaction. Use this when user mentions spending money or receiving money.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description:
          "Transfer money between accounts (e.g., paying credit card from bank account, moving to savings). This is NOT income or expense.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description:
          "Splitwise: Create an expense. Provide either explicit users array or split_equally fields.",
        inputSchema: {
          type: "object",
          properties: {
            cost: { type: "number" },
            ...
            users: {
              type: "array",
              description:
                "Explicit split. Each user: { user_id, paid_share, owed_share }. Numbers as strings are ok.",
              items: {
                type: "object",
                properties: {
                  user_id: { type: "number" },
...
const toolCallHandler = async (request: any) => {
  ...
  if (name.startsWith("splitwise_")) {
    try {
      ...
      if (name === "splitwise_get_friends") {
        ...
        if (!include_balances) return safeJson({ count: friends.length, friends });

        const summarized = friends.map((f: any) => {
      ...
      if (name === "splitwise_get_net_balances") {
        ...
        const owed_to_you: Array<{ friend_id: number; name: string; currency_code: string; amount: number }> = [];
        const you_owe: Array<{ friend_id: number; name: string; currency_code: string; amount: number }> = [];

        const totals: Record<string, { owed_to_you: number; you_owe: number; net: number }> = {};
      ...
      if (name === "splitwise_get_expenses") {
        ...
        if (dated_after) payload.dated_after = dated_after;
        if (dated_before) payload.dated_before = dated_before;
      ...
      if (name === "splitwise_create_expense") {
        ...
        if (typeof payment === "boolean") payload.payment = payment;
      ...
      if (name === "splitwise_update_expense") {
        ...
        const payload: any = { id: expense_id };

        if (cost !== undefined) payload.cost = String(cost);
        if (description !== undefined) payload.description = description;
        if (date !== undefined) payload.date = date;
        if (group_id !== undefined) payload.group_id = group_id;
        if (currency_code !== undefined) payload.currency_code = currency_code;
  ...
  if (name === "add_transaction") {
    ...
    if (error) return safeJson({ error: error.message });
  ...
  if (name === "transfer_between_accounts") {
    ...
    if (!fromAcc) return safeJson({ error: `Account '${from_account}' not found` });
    if (!toAcc) return safeJson({ error: `Account '${to_account}' not found` });
  ...
  if (name === "get_account_balance") {
    ...
    if (account_name) {
      const account = await findAccount(account_name);
      if (!account) return safeJson({ error: `Account '${account_name}' not found` });
  ...
  if (name === "get_transactions") {
    ...
    if (account_name) {
      filtered = filtered.filter((t: any) => t.accounts?.name?.toLowerCase().includes(account_name.toLowerCase()));
    }

    if (search) {
      const sLower = search.toLowerCase();
      filtered = filtered.filter(
        (t: any) => t.description?.toLowerCase().includes(sLower) || t.category?.toLowerCase().includes(sLower)
      );
    }
  ...
  if (name === "edit_transaction") {
    ...
    if (date !== undefined) updates.date = parseDate(date);
  ...
  if (name === "get_summary") {
    ...
    for (const t of expenses) expensesByCategory[t.category] = (expensesByCategory[t.category] || 0) + Number(t.amount);

    const incomeByCategory: Record<string, number> = {};
    for (const t of income) incomeByCategory[t.category] = (incomeByCategory[t.category] || 0) + Number(t.amount);
  ...
  if (name === "compare_spending") {
    ...
    const total1 = (trans1 || []).reduce((sum: number, t: any) => sum + Number(t.amount), 0);
    const total2 = (trans2 || []).reduce((sum: number, t: any) => sum + Number(t.amount), 0);
  ...
  if (name === "get_budget_status") {
    ...
    const results = await Promise.all(
      (budgets || []).map(async (budget: any) => {
        ...
        const spent = (transactions || []).reduce((sum: number, t: any) => sum + Number(t.amount), 0);
        ...
        return {
          ...
          status:
            spent > Number(budget.limit_amount)
              ? "over_budget"
  ...
  if (name === "get_categories") {
    ...
    return safeJson({ categories: categories?.map((c: any) => ({ name: c.name, type: c.type })) });
  }
  ...
  if (name === "get_recurring_due") {
    ...
    return safeJson({
      ...
      recurring_transactions: (recurring || []).map((r: any) => ({
...
app.post("/mcp", async (req, res) => {
  try {
    ...
    if (!id && id !== 0) return res.status(202).end();
